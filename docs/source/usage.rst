...
The principal and
resource can be supplied through Strands ``invocation_state``:
...
Run the checked-in Strands example:
...
This opens an interactive CLI:

.. code-block:: text

   shopping-agent>

The target accepts CLI arguments through ``ARGS``:

.. code-block:: bash

   make strands-shopping-agent ARGS="--user alice --resume-session sess_123 --item-id iphone17"

Use ``--scripted`` for deterministic policy-walkthrough output:

.. code-block:: bash

   make strands-shopping-agent ARGS="--user alice --scripted"

The checkout command evaluates budget and order quota policies alongside
session login, add-to-cart sequencing, approval, and item-risk policies.

Framework-neutral shopping agent policies are also available in
``examples/shopping_agent_policies``. They use the same Dogwood ``CallTool``
event shape but are not tied to Strands, so future LangChain or CrewAI
integrations can use the same policy files. The Strands example above loads
and exercises those shared policies.
